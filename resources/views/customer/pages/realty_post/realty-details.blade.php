...
<div data-src="{{Agent::isMobile() ? getThumbnail($img->link) : $img->link}}" class="item embed-responsive embed-responsive-16by9 position-relative">
@if (Agent::isDesktop())
<div class="blur-bg position-absolute w-100 h-100" style="top: 0;  background: linear-gradient(rgba(0, 0, 0, 0.3), rgba(0, 0, 0, 0.3)), url('{{Agent::isMobile() ? getThumbnail($img->link) : $img->link}}');"></div> 
@endif

<img class="embed-responsive-item rounded lazy" data-src="{{Agent::isMobile() ? getThumbnail($img->link) : $img->link}}" style="object-fit: contain" alt="{{$img->alt}}" title="{{$img->title}}">
</div>
@endforeach
</div>
...
<img class="embed-responsive-item rounded lazy" data-src="{{getThumbnail($img->link)}}" alt="{{$img->alt}}" title="{{$img->title}}">
</div>
@endforeach
</div>
</div>
...
<strong class="font-1">{{$realty->area}}m<sup>2</sup></strong>
</div>
<div class="px-3">
...
@endphp
<img class="py-1 border d-block rounded-circle" width="60px" height="60px" src="{{$author->profile_image_path ?? '/images/empty-avatar.jpg'}}" alt="{{$author->name}}" title="{{$author->name}}">
<div class="pl-3">
...
@foreach ($realty->district->realty_posts->where('status',3)->take(6) as $item)
...
<div class="author-btn-group">
<a class="font-9 font-weight-bold  d-block shadow-5 mt-2 w100 py-2 hrm-btn-info call-to-us-detail" href="tel:+{{$realty_post->contact_phone_number ?? ''}}">SĐT: {{$realty_post->contact_phone_number ??''}}</a>
...
<a style="line-height: 1.8em" class="px-3 font-20 d-block " href="sms:{{$realty_post->contact_phone_number ?? ''}}"><i class="fas fa-sms"></i></a>
<a style="line-height: 1.8em" class="px-3 font-16 d-block " href="https://zalo.me/{{$realty_post->contact_phone_number ?? ''}}"><img src="{{ asset('images/zalo.png') }}" alt="zalo" style="width: 100%" title="zalo"></a>
<a style="line-height: 1.8em" class="font-14 btn btn-success rounded-0 d-block w-100 call-to-us-detail" href="tel:{{$realty_post->contact_phone_number ?? ''}}"><i class="fas fa-phone-alt call-to-us-detail"></i>

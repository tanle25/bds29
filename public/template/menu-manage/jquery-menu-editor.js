...
(function ($) {
    ...
    $.fn.sortableLists = function (options) {
            ...
            $(this)
                ...
                .each(function () {
                    ...
                    if (li.children(setting.listSelector).length) {
                        opener
                            .clone(true)
                            .prependTo(li.children("div").first());
        ...
        function startDrag(e, el, rEl) {
            ...
            ///// Sửa code để giảm vị trí khi kéo element
            state.cEl.xyOffsetDiff = {
                X: e.pageX - state.cEl.offset.left,
                Y: e.pageY - state.cEl.offset.top + 50,
            };
            state.cEl.el.addClass(
                "sortableListsCurrent" + " " + setting.currElClass
            );
        ...
        function dragging(e) {
            if (state.isDragged) {
                ...
                // Scrolling up
                if (
                    doc.scrollTop() > state.rootEl.offset.top - 10 &&
                    e.clientY < 50
                ) {
                    if (!state.upScroll) {
                ...
                // Scrolling down
                    ...
                    doc.scrollTop() + win.height() <
                        state.rootEl.offset.top +
                            state.rootEl.el.outerHeight(false) +
                            10 &&
        ...
        function elFromPoint(x, y) {
            ...
            if (isRelEFP === null) {
                ...
                if ((s = state.doc.scrollTop()) > 0) {
                    isRelEFP =
                        (res = document.elementFromPoint(
                            0,
                            s + $(window).height() - 1
                        )) == null || res.tagName.toUpperCase() == "HTML"; // IE8 returns html
                }
            ...
            if (!state.rootEl.el.find(el).length) {
                // el is outside the rootEl
                return null;
            } else if (
                el.is("#sortableListsPlaceholder") ||
                el.is("#sortableListsHint")
            ) {
                // el is #placeholder/#hint
                return null;
            } else if (!el.is("li")) {
                // el is ul or div or something else in li elem.
                el = el.closest("li");
                return el[0] ? el : null;
            } else if (el.is("li")) {
                // el is most wanted li
                return el;
            }
        }
        ...
        function showOnTop(e, oEl) {
            ...
            state.isAllowed = setting.isAllowed(
                state.cEl.el,
                hint,
                hint.parents("li").first()
            );
        }
        ...
        function open(li) {
            ...
            if (setting.opener.as == "html") {
                ...
                opener
                    ...
                    .removeClass(setting.opener.open);
            } else {
                opener.css(
                    "background-image",
                    "url(" + setting.opener.close + ")"
                );
            }
        }
        ...
        function close(li) {
            ...
            if (setting.opener.as == "html") {
                ...
                opener
                    ...
                    .removeClass(setting.opener.close);
            } else {
                opener.css(
                    "background-image",
                    "url(" + setting.opener.open + ")"
                );
            }
        }
        ...
        function tidyCurrEl(cEl) {
            ...
            cEl.el.removeClass(
                setting.currElClass + " " + "sortableListsCurrent"
            );
            cElStyle.top = "0";
            cElStyle.left = "0";
...
function MenuEditor(idSelector, options) {
    var $main = $("#" + idSelector);
    var settings = {
        labelAdd: '<i class="fas fa-plus clickable"></i>',
    ...
    function TButtonGroup() {
        ...
        var $btnEdit = TButton({
            ...
            text: settings.labelEdit,
        });
        var $btnAdd = TButton({
            classCss: "btn btn-success btn-sm btnAdd",
            text: settings.labelAdd,
        });
        var $btnRemv = TButton({
            classCss: "btn btn-danger btn-sm btnRemove",
            text: settings.labelRemove,
        });
        var $btnUp = TButton({
    ...
    function createMenu(arrayItem, depth) {
        ...
        var $elem =
            level === 0
                ? $main
                : $("<ul>").addClass("pl-0").css("padding-top", "10px");
        $.each(arrayItem, function (k, v) {
            var isParent =
                typeof v.children !== "undefined" && $.isArray(v.children);
            var itemObject = {
                text: "",
                href: "",
                icon: "empty",
    ...
    this.setData = function (strJson) {
        var arrayItem = Array.isArray(strJson)
            ? strJson
            : stringToArray(strJson);
        if (arrayItem !== null) {
            $main.empty();
